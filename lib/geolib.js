// ADAPTED FROM Manuel Bieh's geolib LIBRARY
...
const geolib = Object.create(Geolib.prototype, {
  version: {
    value: '2.0.23'
  },
  radius: {
...
geolib.extend({
  ...
  getKeys: function (point) {
    ...
    let getKey = function (possibleValues) {
    ...
    let longitude = getKey(['lng', 'lon', 'longitude']);
    let latitude = getKey(['lat', 'latitude']);
    let elevation = getKey(['alt', 'altitude', 'elevation', 'elev']);
  ...
  coords: function (point, raw) {
    ...
    let elev = point[this.getKeys(point).elevation];
  ...
  */
  getDistance: function (start, end, accuracy, precision) {
    ...
    let s = this.coords(start);
    let e = this.coords(end);

    let a = 6378137;
    let b = 6356752.314245;
    let f = 1 / 298.257223563; // WGS-84 ellipsoid params
    let L = (e['longitude'] - s['longitude']).toRad();

    let cosSigma, sigma, sinAlpha, cosSqAlpha, cos2SigmaM, sinSigma;

    let U1 = Math.atan((1 - f) * Math.tan(parseFloat(s['latitude']).toRad()));
    let U2 = Math.atan((1 - f) * Math.tan(parseFloat(e['latitude']).toRad()));
    ...
    if (typeof this.elevation(start) !== 'undefined' && typeof this.elevation(end) !== 'undefined') {
      let climb = Math.abs(this.elevation(start) - this.elevation(end));
  ...
  */
  getCenter: function (coords) {
    ...
    let nbCoords = coordsArray.length;
    X = X / nbCoords;
    Y = Y / nbCoords;
    Z = Z / nbCoords;
  ...
  */
  getBounds: function (coords) {
    ...
    let useElevation = this.elevation(coords[0]);
  ...
  */
  getCenterOfBounds: function (coords) {
    let b = this.getBounds(coords);
    let latitude = b.minLat + ((b.maxLat - b.minLat) / 2);
    let longitude = b.minLng + ((b.maxLng - b.minLng) / 2);
    return {
  ...
  */
  getBoundsOfDistance: function (point, distance) {
    let latitude = this.latitude(point);
    let longitude = this.longitude(point);

    let radLat = latitude.toRad();
    let radLon = longitude.toRad();

    let radDist = distance / this.radius;
    let minLat = radLat - radDist;
    let maxLat = radLat + radDist;

    let MAX_LAT_RAD = this.maxLat.toRad();
    let MIN_LAT_RAD = this.minLat.toRad();
    let MAX_LON_RAD = this.maxLon.toRad();
    let MIN_LON_RAD = this.minLon.toRad();
  ...
  */
  ...
  isPointInsideWithPreparedPolygon: function (point, coords) {
    ...
    let y = this.longitude(point);
    let x = this.latitude(point);
  ...
  */
  isInside: function (...args) {
    return this.isPointInside.apply(this, args);
  },
  ...
  */
  withinRadius: function (...args) {
    return this.isPointInCircle.apply(this, args);
  },
  ...
  */
  getDirection: function (...args) {
    return this.getCompassDirection.apply(this, ...args);
  },
  ...
  */
  orderByDistance: function (latlng, coords) {
    ...
    let coordsArray = Object.keys(coords).map(function (idx) {
      let distance = this.getDistance(latlng, coords[idx]);
      let augmentedCoord = Object.create(coords[idx]);
  ...
  */
  getDistanceFromLine: function (point, start, end) {
    let d1 = this.getDistance(start, point, 1, 3);
    let d2 = this.getDistance(point, end, 1, 3);
    let d3 = this.getDistance(start, end, 1, 3);
    let distance = 0;

    // alpha is the angle between the line from start to point, and from start to end //
    let alpha = Math.acos((d1 * d1 + d3 * d3 - d2 * d2) / (2 * d1 * d3));
    // beta is the angle between the line from end to point and from end to start //
    let beta = Math.acos((d2 * d2 + d3 * d3 - d1 * d1) / (2 * d2 * d3));
  ...
  */
  findNearest: function (latlng, coords, offset, limit) {
    offset = offset || 0;
    limit = limit || 1;
    let ordered = this.orderByDistance(latlng, coords);

    return (limit === 1 ? ordered[offset] : ordered.splice(offset, limit));
  },
  ...
  */
  getPathLength: function (coords) {
    ...
    for (let i = 0, l = coords.length; i < l; ++i) {
      if (last) dist += this.getDistance(this.coords(coords[i]), last);
  ...
  */
  getSpeed: function (start, end, options) {
    ...
    let time = ((end.time * 1) / 1000) - ((start.time * 1) / 1000);
    let mPerHr = (distance / time) * 3600;
    let speed = Math.round(mPerHr * this.measures[unit] * 10000) / 10000;
    return speed;
  },
  ...
  computeDestinationPoint: function (start, distance, bearing, radius) {
    let lat = this.latitude(start);
    let lng = this.longitude(start);

    radius = (typeof radius === 'undefined') ? this.radius : Number(radius);

    let δ = Number(distance) / radius; // angular distance in radians
    let θ = Number(bearing).toRad();

    let φ1 = Number(lat).toRad();
    let λ1 = Number(lng).toRad();
  ...
  */
  decimal2sexagesimal: function (dec) {
    ...
    if (dec in this.sexagesimal) return this.sexagesimal[dec];

    let tmp = dec.toString().split('.');

    let deg = Math.abs(tmp[0]);
  ...
  */
  sexagesimal2decimal: function (sexagesimal) {
    ...
    let regEx = new RegExp(this.sexagesimalPattern);
    let data = regEx.exec(sexagesimal);
    let min = 0;
    let sec = 0;
